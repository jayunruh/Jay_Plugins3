...
public class jay_stitching_plugin_jru_v1 implements PlugIn, gui_interface {
	...
	public void run(String arg) {
		...
		for(int i=0;i<pairs.length;i++){
			...
			pairStats[i][0]=bestpeak[1]; pairStats[i][1]=bestpeak[2]; pairStats[i][2]=bestpeak[0]; pairStats[i][3]=bestpeak[4];
			stats[i][4]=pairStats[i][0]; stats[i][5]=pairStats[i][1];
			stats[i][6]=pairStats[i][2]; stats[i][7]=pairStats[i][3];
			IJ.log(""+pair1+ " <- "+pair2+":,"+stats[i][2]+" , "+stats[i][3]+" , "+stats[i][4]+ " , "+stats[i][5]+" , " + " correlation (R)=" + stats[i][6]+" , "+stats[i][7]);
		}
		//new ImagePlus("Norm Correlation Images",ccstack).show();
		//check the coordinates
		//all vertical and horizontal offsets should be within ftoler (10%) of their median positive values
		//if not, set them to the median and zero their correlation values (they are now slave to the other offsets)
		float[] medshifts=s.checkCoords(pairStats,pairs,medshifttoler);
		...
		//float hoverlap=(float)width-Math.abs(medshifts[0]);
		//float voverlap=(float)height-Math.abs(medshifts[3]);
		...
		Object stitched=s.stitch_frame(jutils.stack2array(stack),true,hoverlap,voverlap);
