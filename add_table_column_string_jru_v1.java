...
public class add_table_column_string_jru_v1 implements PlugIn {
	...
	public void run(String arg) {
		...
		if(niframes[index] instanceof TextWindow){
			...
			for(int i=0;i<nlines;i++){
				...
				String temp=func(new_col_labels,arr,formula);
				arr.add(temp);
			}
	...
	public String func(String[] column_names,List<String> column_values,String function){
		StringBuffer script=new StringBuffer();
		for(int i=0;i<column_names.length;i++){
			String temp=column_values.get(i);
			temp.replace("\"","\\\"");
		...
		String temp="";
		try{
			temp=(String)engine.eval(script.toString());
		}catch(Exception e){
			IJ.log(e.getMessage());
		}
		return temp;
