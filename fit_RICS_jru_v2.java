 ...
 * edits for useability relative to v1 were maded by Philippe CARL (philippe.carl@unistra.fr)
 ******************************************************************************/
import ij.*;
import ij.process.*;
import ij.gui.*;
...
public class fit_RICS_jru_v2 implements PlugInFilter, NLLSfitinterface {
	ImagePlus imp;
	double[] params;
	int[] fixes;
	double[] stats;
	...
	public void run(ImageProcessor ip) {
		...
		gd2.addNumericField("X_center",xc,0);
		gd2.addNumericField("Y_center",yc,0);
		gd2.addNumericField("Pts_in_x_to_fit",xpts,0);
		gd2.addNumericField("Pts_in_x_to_skip",xskip,0);
		gd2.addNumericField("Pts_in_y_to_fit",ypts,0);
		gd2.addNumericField("Photons",photons,0);
		gd2.addNumericField("Pixel_Size_(um)",(float)(pixelsize),5,10,null);
		gd2.addNumericField("Pixel_Time_(us)",(float)(pixeltime*1000000.0),5,10,null);
		gd2.addNumericField("Line_Time_(ms)" ,(float)(linetime*1000.0),5,10,null);
	...
	{
		String[] paramsnames={"w0_(um)","z0/w0","Baseline","G(0)_1","D1_(um^2/sec)","G(0)_2","D2_(um^2/sec)"};
		GenericDialog gd = new GenericDialog("Fitting Parameters");
		gd.setLayout(new GridLayout(12, 2));
		gd.addCheckbox("Test_Chi_Squared?",c2test);
		gd.addMessage("");
		gd.addMessage("");
		gd.addNumericField("Max_Iterations?",maxiter,0);
		gd.addMessage("");
		gd.addNumericField("w0_(um)",params[0],10,15,"");
		...
		gd.addNumericField("G(0)_1",params[3],10,15,"");
		gd.addCheckbox("fix?",(fixes[3]==1));
		gd.addNumericField("D1_(um^2/sec)",params[4],10,15,"");
		gd.addCheckbox("fix?",(fixes[4]==1));
		gd.addNumericField("G(0)_2",params[5],10,15,"");
		gd.addCheckbox("fix?",(fixes[5]==1));
		gd.addNumericField("D2_(um^2/sec)",params[6],10,15,"");
		gd.addCheckbox("fix?",(fixes[6]==1));
		gd.addNumericField("Iterations_Completed",(int)stats[0],0);
		gd.addMessage("  ");		
		gd.addNumericField("Chi_Squared",(float)stats[1],5,15,"");
		gd.addMessage("  ");
		gd.showDialog();
		if(gd.wasCanceled()){
			String temp;
			...
			TextWindow tw=jutils.selectTable("RICS_fit_parameters");
			if(tw==null) tw=FitDialog_v2.make_outtable("RICS_fit_parameters",paramsnames);
			tw.append(imagename+"\t"+(float)stats[1]+"\t"+(int)stats[0]+"\t"+params[0]+"\t"+params[1]+"\t"+params[2]+"\t"+params[3]+"\t"+params[4]+"\t"+params[5]+"\t"+params[6]);
			return false;
		}
