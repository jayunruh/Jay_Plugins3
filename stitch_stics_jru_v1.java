...
public class stitch_stics_jru_v1 implements PlugIn, gui_interface {
	...
	public void run(String arg) {
		...
		if(!findshifts){
			float[] overlaps=stitching.getAvgOverlap(new float[][]{coords[0],coords[1]},width,height);
			...
			float[][] guessshifts=new float[pairs.length][2];
			for(int i=0;i<pairs.length;i++){
				guessshifts[i][0]=coords[0][pairs[i][1]]-coords[0][pairs[i][0]];
				guessshifts[i][1]=coords[1][pairs[i][1]]-coords[1][pairs[i][0]];
			}
			//need to find the fft dimension here
			float[] overlaps=stitching.getAvgOverlap(new float[][]{coords[0],coords[1]},width,height);
			...
			for(int i=0;i<pairs.length;i++){
				...
				stats[i][2]=guessshifts[i][0]; stats[i][3]=guessshifts[i][1];
				if(ccstack==null) ccstack=new ImageStack(subfftdim,subfftdim);
				ccstack.addSlice(""+pair1+","+pair2,cc[i]);
				float[][] peaks=s.getCorrSTICSPeaks(cc[i],3,pix[pair1],pix[pair2],guessshifts[i],subfftdim); //these are phasecorr,x,y,crosscorr
				//peaks are ranked in order of decreasing correlation
				//float[] bestpeak=peaks[0];
				float[][] bestpeak=findBestPeak(peaks,guessshifts[i],peaksel);
			...
			overlaps=stitching.getAvgOverlap(new float[][]{newxvals,newyvals},width,height);
			float hoverlap=overlaps[0]; float voverlap=overlaps[1];
			IJ.log("overlaps: "+hoverlap+" , "+voverlap);
