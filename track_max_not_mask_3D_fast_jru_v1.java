...
public class track_max_not_mask_3D_fast_jru_v1 implements PlugIn, DialogListener,  track_interface{
	...
	public void run(String arg) {
		...
		fb=new FindBlobsFast(width,height,256,new float[]{0.0f,10000.0f,0.0f,1000.0f,10,5,10});
		...
		if(counter>0){
			...
			if(frames==1) {
				int[] shapes=pw.getShapes();
				for(int i=0;i<shapes.length;i++) shapes[i]=1;
			}
			pw.draw();
		}
	}

	public float[] get_criteria(ImagePlus imp,FindBlobsFast fb){
	...
	public List<float[]> getFrameParams(int frame){
		...
		float[][] blobstats=fb.dofindblobs3D(pixels,searchrz,zedgebuf);
