...
*
*Jay Unruh converted this to a plugin and added the z scaling ratio code so that z distances are in xy pixel units
 */
public class test_sedt_jru_v1 implements PlugIn {
	ImagePlus image;
	int w, h, d;
	int current, total;
	...
	public void run(String arg) {
		...
		compute(image.getStack(),zratio).show();
	}

	public ImagePlus compute(ImageStack stack,float zratio) {
		w = stack.getWidth();
		h = stack.getHeight();
		d = stack.getSize();
		...
		for(int i=0;i<d;i++){
			float[] temp=(float[])result.getPixels(i+1);
			for(int j=0;j<w*h;j++) temp[j]*=zratio;
		}
